...
```
The variables that are avaiable are Name, sex, count, and year. A case represnts a male baby name for a year.
### Step 2
...
Sex because only male names are shown. 
...
No variables appear to be transformed because the only variables that exist are year, name, and count. 
...
Male names that are not Abraham, Woodrow, or Franklin.


##### Have cases been grouped and summarized within groups in any way?

They were grouped by name and year then the count for each year was summarized. 


##### Have any new variables been introduced?  If so, what the relationship between the new variables and existing variables?

No new variables have been introduced and no variables have been mutated. 
...
GlyphReadyForm<-BabyNames%>%
  filter(name=="Linda"|name=="James"|name=="Michael"|name=="James"|name=="John")%>%
  group_by(name,year)%>%
